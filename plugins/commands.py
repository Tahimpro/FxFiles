...
async def shortlink(bot, message):
    ...
    LOG_CHANNEL,
    text=script.SHORTNERADD.format(
        message.text.replace("/set_shortner", "").replace("@PantherF_Robot", ""),
        title,
        grpid,
...
async def settutorial(bot, message):
    ...
    if len(message.command) == 1:
        ...
        await bot.send_message(
    LOG_CHANNEL,
    text=script.TUTORIALADD.format(
        message.text.replace("/set_tutorial", "").replace("@PantherF_Robot", ""),
        title,
        grpid,
        message.from_user.mention
    ),
