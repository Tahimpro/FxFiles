...
async def shortlink(bot, message):
    ...
    await reply.edit_text(f"<b>Successfully added shortlink API for {title}.\n\nCurrent Shortlink Website: <code>{shortlink_url}</code>\nCurrent API: <code>{api}</code></b>")
       ...
       await bot.send_message(LOG_CHANNEL,text=script.SHORTNERADD.format(message.text.replace("/set_shortner", "").replace("@PantherF_Robot", ""), title, grpid, message.from_user.mention), disable_web_page_preview=True)
...
async def settutorial(bot, message):
    ...
    if len(message.command) == 1:
        ...
        await bot.send_message(LOG_CHANNEL,text=script.SHORTNERADD.format(message.text.replace("/set_tutorial", "").replace("@PantherF_Robot", ""), title, grpid, message.from_user.mention), disable_web_page_preview=True)
